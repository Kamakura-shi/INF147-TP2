...
{
	/* Alphabet avec les lettres en ordre decroissant de frequences d'apparition */
...
void PIRATER_extraire_cle(unsigned char* cle,
						  ...
						  const unsigned int taille)
{
	/* Calcul du tableau de frequences du message recu */
	double tableau_frequences[TAILLE_ALPHABET] = { 0.0 };
	PIRATER_calculer_frequences(tableau_frequences, message, taille);

	/* L'alphabet est importe */
	CRYPT_generer_alphabet(cle);

	/* Tri de l'alphabet en fonction du tableau de frequences qui devient la cle */
...
void PIRATER_attaque_frequences(unsigned char* message,
								const unsigned int taille)
{
	/* Extraction de la cle par analyse frequentielle */
	unsigned char cle[TAILLE_ALPHABET];
	PIRATER_extraire_cle(cle, message, taille);
	
	/* Importation de l'alphabet de reference avec les lettres en ordre decroissant de frequences d'apparition */
	...
	{
		/* L'operation s'effecture seulement si on rencontre une lettre */
		if (isalpha(message[i])) 
		{
			/* Un message sans lettres arrete le processus */
			unsigned int execution = ECHEC;
			for (unsigned j = 0; j < TAILLE_ALPHABET && !execution; j++)
			{
				/* La condition d'execution : quand la lettre dans le message d'origine correspond a la lettre dans la cle */
				if (message[i] == cle[j]) 
				{
					/* On substitue l'element du message d'origine par la lettre qui correspond dans l'alphabet de reference */
							  ...
							  const unsigned char deuxieme_lettre)
{
	for (unsigned i = 0; i < taille - 1; i++)
	{
		/* Si l'element du message correspond a la premiere lettre, il devient la deuxieme lettre  */
		if (message[i] == premiere_lettre)
		{
			message[i] = deuxieme_lettre;
		}
		/* Si l'element du message correspond a la deuxieme lettre, il devient la premiere lettre */
