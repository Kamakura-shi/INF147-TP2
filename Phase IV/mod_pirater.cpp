...
void PIRATER_extraire_cle(unsigned char* cle,
						  ...
						  const unsigned int taille)
{
	double tableau_principal[TAILLE_ALPHABET];
	PIRATER_calculer_frequences(tableau_principal, message, taille);

	CRYPT_generer_alphabet(cle);

	OUTILS_tri_decroissant_etendu_dbl_uchar(tableau_principal, cle, taille);
}
								...
								const unsigned int taille)
{
	...
	PIRATER_generer_alphabet_reference(cle);
