											 ...
											 const unsigned int taille_tableau)
{
	...
	for (unsigned int i = 0; i < taille_tableau - 1; i++)
	{
		...
		{
			/* Si la valeur courante est plus grande que la reference, je permute */
			if (tableau_principal[j] > tableau_principal[i_max])
			{
				i_max = j;
			}

			/* Permuter la valeur a gauche completement avec la plus grande trouvee */
			SWAP_DBL(tableau_principal[i], tableau_principal[i_max])
			SWAP_INT(tableau_secondaire[i], tableau_secondaire[i_max])
		}
	}
}
