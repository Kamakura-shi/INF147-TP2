                                             ...
                                             const unsigned int taille) 
{
	...
	unsigned int i_max = 0;              /*Ici on considere que la position  de la valeur max est à l'indice 0*/

    for (int i = 1; i < taille; i++)     /* Ici on parcourt tout le tableau */
    {
        if (tableau[i] > tableau[i_max]) /* Comparaison de la valeur de chaque position avec i_max */
        i_max = i;        
    }
    return i_max;                         /* on retourne la position de la valeur max  */
}
